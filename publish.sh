...
if [ "$1" == "server" ]; then
    ...
    ssh -t $host "export DOXIE_OPENAI_KEY=$DOXIE_OPENAI_KEY && export DOXIE_ADMIN_TOKEN=$DOXIE_ADMIN_TOKEN && export DOXIE_DB_PASSWORD=$DOXIE_DB_PASSWORD && cd $host_dir && ./docker/control.sh stop && ./docker/control.sh start && ./docker/control.sh logs"
else
